...
namespace Movies.Tests.Unit.BLL
{
    public class MovieBLLUnitTests
    {
        ...
        public async Task GetByIdAsync_WhenMovieDoesNotExist_ThrowsMovieNotFoundException()
        {
            ...
            var exception = await Assert.ThrowsAsync<MovieNotFoundException>(() => _bll.GetByIdAsync(movieId));

            Assert.IsType<MovieNotFoundException>(exception);

            _mockRepository.Verify(x => x.GetByIdAsync(movieId), Times.Once);
            _mockMapper.Verify(x => x.Map<MovieDTO>(movieEntity), Times.Never);
        }
        #endregion
    }
}
