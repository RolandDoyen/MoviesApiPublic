...
- **Movies.Tests** :
  ...
  - **Unit/** : Tests unitaires isolés pour chaque composant.
    ...
    - `Middleware` : Test de la logique isolée du middleware (journalisation, manipulation du contexte) en simulant RequestDelegate et ILogger.
...
- Ce projet utilise le Versionnage par URI pour garantir la rétrocompatibilité.
- v2 (Actuelle) : Inclut des structures de données optimisées et les derniers correctifs de sécurité.
- v1 (Obsolète) : Toujours active mais ne reçoit plus de mises à jour. Sera supprimée dans les futures versions.
- Note d'implémentation : L'obsolescence est signalée dans les réponses de l'API via un en-tête HTTP personnalisé X-API-Deprecated.
