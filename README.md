...
- **Movies.Tests**: 
  ...
  - **Unit/**: Isolated logic validation.
    ...
    - `Middleware`: Testing isolated middleware logic (logging, context manipulation) by mocking RequestDelegate and ILogger.
...
- This project implements URI Versioning to ensure backward compatibility.
- v2 (Current): Includes optimized data structures and the latest security patches.
- v1 (Deprecated): Still active but no longer receiving updates. It will be decommissioned in future releases.
- Implementation note: Deprecation is signaled in the API responses via a custom X-API-Deprecated HTTP header.
